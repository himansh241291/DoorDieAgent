...
class MarketIntelligence:
    ...
    def _returns(closes: list[float]) -> list[float]:
        ...
        for previous,current in zip(closes,closes[1:]):
            if previous<=0 or current<=0 or not math.isfinite(previous) or not math.isfinite(current): return []
    ...
    def daily_bars(bars:list[Bar])->list[Bar]:
        ...
        for bar in sorted(bars,key=lambda x:x.end): groups.setdefault(bar.end.astimezone(IST).date(),[]).append(bar)
        daily=[]
    ...
    def benchmark(self,bars:list[Bar])->dict[str,float|bool|None]:
        closes=self._closes(self.daily_bars(bars)); valid=[v for v in closes if v>0 and math.isfinite(v)]
        ...
        for end in range(self.volatility_window,len(returns)+1):
            window=returns[end-self.volatility_window:end]; avg=mean(window); realized.append(math.sqrt(mean((v-avg)**2 for v in window))*math.sqrt(252.0))
        percentile=None
        if len(realized)>=self.volatility_history:
            current=realized[-1]; history=realized[-self.volatility_history:]; percentile=sum(v<=current for v in history)/len(history)
        vol_shock=None
        if len(realized)>=2:
    ...
    def breadth(self,bars_by_symbol:dict[str,list[Bar]])->float|None:
        ...
        for bars in bars_by_symbol.values():
            closes=self._closes(self.daily_bars(bars)); valid=[v for v in closes if v>0 and math.isfinite(v)]; sma20=self._sma(valid,self.breadth_window)
            if sma20 is None or not closes or closes[-1]<=0 or not math.isfinite(closes[-1]): continue
