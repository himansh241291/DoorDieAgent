...
class TradingAgent:
    ...
    def _market_intelligence(self,symbols,now):
        ...
        for symbol in symbols:
            bars_by_symbol[symbol]=self.provider.completed_bars(symbol,interval,now)
    ...
    def cycle(self, symbols:list[str]):
        ...
        if not session.exits_allowed:
            event(self.log,logging.INFO,"market_session_closed",ts_ist=ist.isoformat(),state=session.state.value); return
        ...
        for symbol,p in list(self.repo.positions().items()):
            ...
            if not exit_ok:
                self.repo.record_risk(now,"EXIT_PRICE_UNAVAILABLE",False,exit_reason,{"symbol":symbol}); continue
            ...
            if bid<=p.stop_price:
                ...
                self._notify("paper_exit",symbol=symbol,reason="STOP",price=str(f.price),qty=f.qty,net_pnl=None); continue
            if bid>=p.target_price:
                f=self.broker.sell(symbol,q,now,p.strategy_version,ExitReason.TARGET)
                self._notify("paper_exit",symbol=symbol,reason="TARGET",price=str(f.price),qty=f.qty,net_pnl=None); continue
        ...
        if session.state.value == "EOD" and session.is_trading_day:
            ...
            if last_eod != eod_date:
                daily_start=self.repo.db.get_state("daily_start_equity",self.cfg["account"]["starting_capital"]); gross=equity-self.repo.cash()
                previous_marks=self.repo.eod_marks(self.cfg["risk"]["rolling_drawdown_days"]-1); values=[float(mark["equity"]) for mark in reversed(previous_marks)]; peak=max(values+[equity]) if values else equity; drawdown=(peak-equity)/peak if peak>0 else 0.0
        ...
        try:
            regime,metrics,benchmark_bars,market_bars=self._market_intelligence(symbols,now)
        except Exception as exc:
            self.repo.record_data_health(now,False,"market_intelligence_failure",{"error":str(exc)})
            regime=self.regime_engine.classify(now,None,None,None,None,None,None,False)
            metrics={}; benchmark_bars=[]; market_bars={}
        ...
        for symbol in symbols:
            ...
            try:
                ...
                self._notify("paper_entry",symbol=symbol,price=str(f.price),qty=qty,stop=str(p.stop_price),target=str(p.target_price))
            except Exception as exc:
