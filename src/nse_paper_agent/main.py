...
from nse_paper_agent.config import immutable_risk_hash, load_yaml, validate_runtime_config
...
def build(cfg_path):
    ...
    expected = cfg["safety"].get("production_config_immutable_risk_hash", "")
    actual = immutable_risk_hash(cfg)
    if expected and expected != actual:
        raise ValueError("immutable risk configuration hash mismatch")
    ...
    health = DataHealth(30, cfg["risk"]["max_spread_bps"])
    ...
    regime = RegimeEngine()
    sentiment = NeutralSentimentProvider()
...
def main():
    ...
    ap.add_argument("--config", default="config/production.paper.yaml")
    ap.add_argument("--symbols", nargs="*", default=[])
    args = ap.parse_args()
    agent, db = build(args.config)
    stop = False
