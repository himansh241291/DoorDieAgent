...
from nse_paper_agent.domain.models import ExitReason, Quote
...
def record_eod(repo, risk, now):
    ...
    equity = risk.equity({}); gross = equity - repo.cash()
...
def main():
    ...
    broker = PaperBroker(cfg, repo); risk = RiskEngine(cfg, repo); strategy = BaselineBreakoutStrategy(); session = SessionGuard(cfg); regime_engine = RegimeEngine()
    intelligence = MarketIntelligence(**{key: value for key, value in cfg["market"]["regime"].items() if key in {"benchmark_min_bars","volatility_window","volatility_history","breadth_window"}}, breadth_min_symbols=cfg["market"]["minimum_breadth_symbols"])

    history = {symbol: repo.market_bars(symbol) for symbol in all_symbols}
    ordered_bars = sorted(bars, key=lambda item: item.end)
    ...
    for bar in ordered_bars:
        ...
        quote = Quote(bar.symbol,now,bar.close * Decimal("0.999"),bar.close * Decimal("1.001"),bar.close,bar.volume); repo.record_quote(quote)
        ...
        if bar.symbol in symbols and session_state.entries_allowed and len(history[bar.symbol]) >= 35:
            benchmark_bars = history.get(benchmark_symbol, [])
            market_bars = {symbol: history[symbol] for symbol in symbols}
            metrics = intelligence.calculate(benchmark_bars,market_bars)
            healthy = True
            regime = regime_engine.classify(now,metrics.get("close"),metrics.get("sma20"),metrics.get("sma50"),metrics.get("breadth20"),metrics.get("vol_percentile"),metrics.get("vol_shock"),healthy)

            signal = strategy.evaluate(history[bar.symbol],now,regime.regime,0.5,bar.symbol in repo.positions(),repo.in_cooldown(bar.symbol,now),True,True)
            ...
            if signal.eligible and bar.symbol not in repo.positions():
                equity = risk.equity(quotes)
                decision = risk.can_buy(now,quotes,equity,regime.regime)
                repo.record_risk(now,"ENTRY",decision.allowed,decision.reason,{"symbol":bar.symbol,"score":signal.score,"regime":regime.regime.value})
                if decision.allowed:
