...
def context_for(entry: Entry, bars: list[Bar]) -> Context:
    ...
    volume_ratio = (
        float(current.volume) / float(previous.volume)
        if current is not None and previous is not None and float(previous.volume) > 0
        else None
    )
            ...
            print(
                ...
                *[
                    ...
                    for key in (
                        "trades",
                        "forward_60m_net_pnl",
                        "forward_60m_expectancy",
                        "forward_60m_positive_pct",
                        "forward_30m_median_pct",
                        "forward_60m_median_pct",
