...
""", encoding="utf-8")
    db = Database(str(db_path)); db.initialize(); repo = Repository(db); repo.set_cash(48980.0)
...
""", encoding="utf-8")
    ...
    db = sqlite3.connect(db_path)
    before = {"bars": db.execute("SELECT COUNT(*) FROM market_bars").fetchone()[0], "fills": db.execute("SELECT COUNT(*) FROM simulated_fills").fetchone()[0], "eod": db.execute("SELECT COUNT(*) FROM account_snapshots WHERE is_eod=1").fetchone()[0]}
    db.close()
    subprocess.run([sys.executable, "scripts/run_replay.py", "--bars", str(bars_path), "--db", str(db_path)], check=True, capture_output=True, text=True)
    db = sqlite3.connect(db_path)
    after = {"bars": db.execute("SELECT COUNT(*) FROM market_bars").fetchone()[0], "fills": db.execute("SELECT COUNT(*) FROM simulated_fills").fetchone()[0], "eod": db.execute("SELECT COUNT(*) FROM account_snapshots WHERE is_eod=1").fetchone()[0]}
    assert after == before
    assert after["eod"] == 1
    db.close()
