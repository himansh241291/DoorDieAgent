...
def finding(code: str, severity: str = "MEDIUM") -> DiagnosisFinding:
...
def test_proposal_engine_converts_diagnosis_to_data_only_proposals():
    diagnosis = StrategyDiagnosis("baseline-breakout-v1", True, (finding("REGIME_WEAKNESS"),))
    proposals = StrategyProposalEngine().propose(diagnosis)

    assert len(proposals) == 1
    proposal = proposals[0]
    assert proposal.base_version == "baseline-breakout-v1"
    assert proposal.proposed_version == "baseline-breakout-v1-challenger-2"
...
def test_proposals_are_bounded():
    ...
    assert proposals[0].proposal_id == "a:A:1"
    assert proposals[1].proposal_id == "a:B:2"
