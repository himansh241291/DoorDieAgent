...
Example for the benchmark plus five liquid NSE cash-equity symbols:

```bash
mkdir -p data
python scripts/download_fyers_history.py \
  --symbols 'NSE:NIFTY50-INDEX,NSE:RELIANCE-EQ,NSE:TCS-EQ,NSE:HDFCBANK-EQ,NSE:ICICIBANK-EQ,NSE:INFY-EQ' \
  --start 2025-09-15 \
  --end 2026-09-14 \
...
The downloader writes the file atomically and reports the returned candle count and zero-volume count per symbol. Zero volume is reported rather than silently fabricated because market-data quality must be measured before replay conclusions are trusted.
